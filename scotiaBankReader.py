...
def nextPurchaseItem(aNumber):
    ...
    print ("Counter is %d" % (nextPurchaseItem.counter -1))
...
def collectInfo(index,lines):
    ...
    while (not isReferenceNumber(lines[index])):
        ...
        print("The current string: %s and the newWord %s and the nextWord after that is %s"  % (string,newWordToEnter, nextWordTobeEnterAfter))
        ...
        if not isReferenceNumber(nextWordTobeEnterAfter):
            ...
            price= lines[index]
            print("this is the price of the item %s" % price)
    print("This is your string %s" % string)
    return index
...
def parsePDF():
    ...
    for line in lines:
        ...
        if isReferenceNumber(line):
            ...
            print("This is the TransMonth: %s , transDay:%s , postMonth:%s, postDay:%s "% (transMonth, transDay, postMonth , postDay))
